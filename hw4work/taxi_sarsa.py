...
def sarsa_algo(alpha,gamma,epislon):
    numEpisodes = 10000
    ...
    for i in range(numEpisodes):
        ...
        while 1:
            ...
            # get the associated next action for the next_state using epislon greedy policy
            next_state_select_action = Q_epislon_greedy_pick_action(Qvals,next_state,epislon)
            # update the Q values based on formula
...
def writeToPickleFiles(Qvals):
    """which contains the dictionary sarsa q vals where the keys are
    ...
    for state in range(500):
        ...
        sarsa_q_vals[state] = temp 
        policy[state] = np.argmax(Qvals[state])
    
    #print(sarsa_q_vals)
    #print(policy)

    with open("sarsa_q_vals.pickle", "wb") as fileToWrite:
        pickle.dump(sarsa_q_vals,fileToWrite)
    
    with open("sarsa_policy.pickle","wb") as fileToWrite: 
...
def plotFigure(rewards):
    ...
    plt.title("Sarsa Algo plot for hw")
    plt.legend()
    plt.savefig("sarsa_total_reward.png")


if __name__ == "__main__":
    Qvals,rewards = sarsa_algo(0.1,0.9,0.1)
